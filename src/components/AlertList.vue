<template>
  <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
    <div class="px-6 py-4 border-b border-slate-200 flex-shrink-0">
      <h3 class="text-lg font-semibold">待处理告警</h3>
    </div>
    ...
    <div class="flex-1 overflow-y-scroll">
      <div class="divide-y divide-slate-200">
        <div
          ...
          :key="alert.id"
          class="px-5 py-2.5 hover:bg-slate-50 transition-colors"
        >
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0 truncate text-sm font-medium text-foreground">
              {{ alert.project }}
            </div>
            ...
            <div class="flex-shrink-0 text-sm text-muted-foreground w-12 text-right">
              {{ alert.time }}
            </div>

            <button class="flex-shrink-0 text-xs text-blue-600 hover:text-blue-700 hover:underline font-medium">
              处理
            </button>
          </div>
        </div>
